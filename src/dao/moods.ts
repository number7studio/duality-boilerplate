...
export const addMoodEntryForUser = async (moodEntries: APIMoodEntryItem[], userId: number): Promise<MoodEntryResponse> => {

    const moodEntry = await MoodEntry.create({
        userId
    });

    const savedMoodItems = await Promise.all(moodEntries.map(({mood, rating}) => {
...
const presentMoodEntry = (moodEntry: MoodEntry): MoodEntryResponse => {
    return {
        [moodEntry.id]: {
            ...
            moodEntryItems: moodEntry.moodEntryItems.sort((a,b) => a.mood > b.mood ? 1 : -1) 
        }
    }
}

export const getMoodEntry = async (userId, moodEntryId): Promise<MoodEntryResponse> => {
...
export const getMoodEntries = async (userId, limit=5): Promise<MoodEntryResponse> => {
