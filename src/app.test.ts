...
test('Signup and login', async () => {
    ...
    const userResponse = await request(app.callback()).post('/auth/signup')
        .send({ "username": "foo", "password": "value2" })
        .set('Accept', 'application/json')

    expect(userResponse.status).toBe(200);

    const loginResponse = await request(app.callback()).post('/auth/login/email')
        .send({ "username": "foo", "password": "value2" })
        .set('Accept', 'application/json')
...
describe('Mood', () => {
    ...
    const createMoodEntry = async (moodEntry) => {
        return request(app.callback()).post('/moods')
            .send({ "moods": [{ mood: "happiness", rating: 5 }, { mood: "anxiety", rating: 1 }] })
            .set('Accept', 'application/json')
            .set('x-auth-token', token);
    }
    ...
    test('Creating Mood entries', async () => {
        const response = await createMoodEntry([{ mood: "happiness", rating: 5 }, 
    ...
    test('Get mood entries', async () => {
        ...
        for (let index = 0; index < 10; index++) {
            await createMoodEntry([{ mood: "happiness", rating: 5 }, 
            { mood: "anxiety", rating: 1 }]);
        }

        const response = await request(app.callback()).get('/moods')
            .set('Accept', 'application/json')
            .set('x-auth-token', token);
